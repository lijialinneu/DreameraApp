...
import neu.dreamerajni.utils.clustering.MarkerManager;
import neu.dreamerajni.utils.clustering.projection.Point;
import neu.dreamerajni.utils.clustering.projection.SphericalMercatorProjection;
import neu.dreamerajni.utils.clustering.ui.IconGenerator;
import neu.dreamerajni.utils.clustering.ui.SquareTextView;

import static neu.dreamerajni.utils.clustering.algo.NonHierarchicalDistanceBasedAlgorithm.MAX_DISTANCE_AT_ZOOM;
            ...
            if (!shouldRenderAsCluster(cluster)) {
                for (T item : cluster.getItems()) {
                    ...
                    if (marker == null) {
                        ...
                        OverlayOptions textOptions = new TextOptions()//添加文字标注
                                .position(item.getPosition())
                                .text(item.getName())
                                .fontSize(42)
                                .fontColor(Color.rgb(0, 150, 64));
                        marker = mClusterManager.getMarkerCollection().addMarker(markerOptions, textOptions);

                        //绑定marker的数据
                        Bundle bundle = new Bundle();
                        bundle.putSerializable("name", item.getName());
                        bundle.putSerializable("text", item.getText());
                        bundle.putSerializable("cross_pictures", item.getCrossPictures());
